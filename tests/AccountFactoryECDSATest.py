...
class TestAccountECDSA():
    ...
    def testCreateFromPublic(self):
        seed = 'divert manage prefer child kind maximum october hand manual connect fitness small symptom range sleep'
        account = AccountFactoryECDSA('T').createFromSeed(seed)
        account2 = AccountFactoryECDSA('T').createFromPublicKey(account.publicKey)
        # object
        ...
        account3 = AccountFactoryECDSA('T').createFromPublicKey(publicKey)
        assert account.address == account3.address
        assert account.publicKey == account3.publicKey
        # b58 str
        account4 = AccountFactoryECDSA('T').createFromPublicKey(base58.b58encode(publicKey))
    ...
    def testCreateFromPrivateKey(self):
        seed = 'divert manage prefer child kind maximum october hand manual connect fitness small symptom range sleep'
        account = AccountFactoryECDSA('T').createFromSeed(seed)
        account2 = AccountFactoryECDSA('T').createFromPrivateKey(account.privateKey)
        # object
        assert account.address == account2.address
        ...
        account3 = AccountFactoryECDSA('T').createFromPrivateKey(privateKey)
        assert account.address == account3.address
        assert account.privateKey == account3.privateKey
        assert account.publicKey == account3.publicKey
        # b58 str
        account4 = AccountFactoryECDSA('T').createFromPrivateKey(base58.b58encode(privateKey))
        assert account.address == account4.address
